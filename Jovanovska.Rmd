...
Elena Jovanovska^c^, Torsten Hauffe,
...
Ecological communities are structured through the interplay of deterministic assembly processes such as species competition and environmental filtering. Whereas the drivers of spatial community structures are frequently studied in extant communities, little is known about the relative importance of deterministic assembly processes in response to environmental factors over evolutionary timescales. Here, we use an integrative framework to unravel drivers and dynamics of community assembly processes since the inception of a long-lived lake ecosystem. 
...
We constructed a dated molecular phylogeny of extant and extinct planktonic diatoms and collected trait data for 380 fossil communities to quantify phylogenetic community and functional structures and to determine the relative importance of deterministic assembly processes over time. We then used generalized additive regression models to correlate the phylogenetic community structure with climatic and environmental variables and to identify primary drivers of assembly processes.

<br>

#### Results

Our results suggest denser packing of niche space with higher species richness and co-occurrence of phylogenetically closely related species, with only two short episodes dominated by distantly related taxa. We found support for additional distinct changes in phylogenetic community structure upon speciation or (local)extinction events and a decrease in mean community relatedness with decreased vegetation cover and a lower precipitation/evaporation ratio during glacial periods.

<br>

#### Main conclusions

Our finding implies filtering as the primary assembly mechanism and a minor importance of competition. The relative importance of these assembly processes was mainly influenced by evolutionary dynamics related to changes in trait distribution of the assemblage and species diversification or population growth, whereas recent glacial periods slightly intensified competition. This study challenges the predictions from ecological theories that competition intensifies as insular ecosystems age. 

<br>

With this html document all steps of the statistical analysis performed in the manuscript are reproduced.
...
plot(SesFric ~ Sr, xlab = "Species richness", ylab = "SES FRic",
...
hist(FricSrRar, xlim = c(-0.35, -0.15), main = "",
     xlab = "Correlation species richness and SES.FRic")
...
Environmental and climate change could influence assembly processes but also intrinsic population dynamics or speciation and extinction events. The following proxies representing local or regional paleoenvironmental changes obtained from DEEP-5045-1 were considered for the analyses of phylogenetic community structure: total inorganic carbon (TIC), total organic carbon (TOC), potassium (K) counts from X-ray fluorescence (XRF) scanning, relative sedimentary quartz content, percentages of arboreal pollen excluding Pinus pollen, percentage of pollen from deciduous oaks (Wagner et al., 2019), oxygen isotope composition of the lake water ($\delta^18$O<sub>lakewater</sub>) and grain size (Wilke et al., 2020). Signals were separated from stochastic noise for all proxies using Gaussian process (gp) smoothing, which is a machine learning approach to identify trends in paleoenvironmental variables as smooth functions of time (Simpson, 2018). We inferred the trend with the gp implementation of brms. We accounted for heteroscedasticity within all Bayesian models via weights representing the time elapsed between consecutive samples in the sediment record (Simpson, 2018). Since temporal autocorrelation might bias model coefficients, we tested for autocorrelation in the model residuals using the Moran’s I index for irregularly-spaced time series (see Legendre and Gauthier, 2014).
...
Threads <- 3
Sys.setenv(STAN_NUM_THREADS = Threads)
BrmOaks <- brm(DecOaks | trials(Total) + weights(Weight) ~ gp(Age, k = K, c = 5/4, gr = FALSE),
               data = Pollen,
               family = binomial(),
               inits = Inits,
               chains = Nchains, cores = Ncores,
...
MoranToc <- correlogFork(time = TocToc$Age[!is.na(TicToc$TOC)],
                          z = ResidToc[, 1], method = "Moran", breaks = Breaks)
plot(MoranToc)
```

Multivariate grain size composition was modelled in brms using the dirichlet distribution. We obtained the temporal trend by summerizing the model-predicted grain-size composition via the first principle component of the  isometric logarithmic ratio-transformed data using the R packages robCompositions 2.3.1 (Filzmoser et al., 2018).
...
PredGrainsize <- predict(BrmGrainsize)
...
We retained the following predictors for the analysis of phylogenetic community structure while excluding the remaining ones due to their collinearity with retained predictors: grainsize, insolation, isotope composition, Potassium, Medstack, deciduous oaks and TOC.
...
To assess impacts on assembly processes, we used regression analysis to examine the relationship between phylogenetic community structure and long-term climate, local paleoenvironmental parameters. We allowed for discrete shifts in community structure at a moment in time as well as including time as a predictor to capture potential intrinsic dynamics and temporal changes. Regression analysis was performed utilizing Bayesian inference in stan 2.21 (Stan Development Team, 2021), which we interfaced through the R package brms 2.15.0 (Bürkner, 2018). Because the number of these discrete shifts in phylogenetic community structure cannot be inferred they were estimated with a change point analysis in the R package ecp 3.1.0 (James & Matteson, 2015) while our Bayesian analysis quantifed the uncertainty in the age and magnitude of the shifts. These shifts in phylogenetic community structure and a potential change over time do not tell the nature of the change and intrinsic dynamics of the assemblage are only one possible interpretation.
...
P1 <- as.data.frame(ggs(FitMpd, burnin = TRUE))
S1 <- P1$value[P1$Parameter == "b_a1_Intercept"]
S2 <- P1$value[P1$Parameter == "b_a2_Intercept"]
S3 <- P1$value[P1$Parameter == "b_a3_Intercept"]
S4 <- P1$value[P1$Parameter == "b_a4_Intercept"]
S5 <- P1$value[P1$Parameter == "b_a5_Intercept"]
S6 <- P1$value[P1$Parameter == "b_a6_Intercept"]
...
mtext(text = "(a)", side = 2, line = 2.5, at = par("usr")[4] - diff(par("usr")[3:4]) * 0.05,
      font = 2, cex = 1.3)
legend(x = 0, y = 5.5, bty = "n", legend = c("Sand", "Very coarse silt", "Coarse silt"),
       fill = c("red3", "red", "orange2"))
legend(x = 2, y = 5.5, bty = "n", legend = c("Medium silt", "Fine silt", "Very fine silt"),
       fill = c("orange", "yellow3", "yellow2"))
legend(x = 4, y = 5.5, bty = "n", legend = "Clay", fill = "yellow")
...
for (i in 1:nrow(ObsGrainsizeSum)) {
  points(rep(Data$Age[i], 6), ObsGrainsizeSum[i, 1:6],
...
plot(0, 0, type = "n", xlim = c(-1400, 0), ylim = c(-11, -1),
     ...
     xaxs = "i", yaxs = "i", xaxt = "n")
mtext(text = "(b)", side = 2, line = 2.5, at = par("usr")[4] - diff(par("usr")[3:4]) * 0.05,
      font = 2, cex = 1.3)
...
plot(Comm$Age, SesFric, type = "n", xlim = c(-1400, 0), ylim = c(-2, 2),
     ...
     xaxs = "i", yaxs = "i", xaxt = "n")
mtext(text = "(c)", side = 2, line = 2.5, at = par("usr")[4] - diff(par("usr")[3:4]) * 0.05,
      font = 2, cex = 1.3)
plotLR04(LR04, Comm$Age)
...
plot(Comm$Age, SesMpd, type = "n", xlim = c(-1400, 0), ylim = c(-2, 2),
     ...
     xaxs = "i", yaxs = "i", xaxt = "n")
mtext(text = "(d)", side = 2, line = 2.5, at = par("usr")[4] - diff(par("usr")[3:4]) * 0.05,
      font = 2, cex = 1.3)
plotLR04(LR04, Comm$Age)
...
points(Comm$Age[-1], SesMpd[-1],
       bg = ColShift[-1], col = "grey60", pch = 21, cex = 0.7)
axis(side = 1, at = seq(-1400, 0, 200),
...
```{r Figure5, cache = TRUE, warning = FALSE, fig.width = 3, fig.height = 9, eval = TRUE}
