...
# Get smoothed values of aboreal pollen proportion for the same moments
# in time where we have a diatom assemblage.
...
```{r Oaks, cache = TRUE, warning = FALSE, eval = FALSE}
...
```{r Quartz, cache = TRUE, warning = FALSE, eval = FALSE}
...
```{r CaK, cache = TRUE, warning = FALSE, eval = FALSE}
CaK <- CaK[nrow(CaK):1, ]
...
```{r Isotope, cache = TRUE, warning = FALSE, eval = FALSE}
...
```{r Tic, cache = TRUE, warning = FALSE, eval = FALSE}
...
```{r Toc, cache = TRUE, warning = FALSE, eval = FALSE}
Threads <- 3
Sys.setenv(STAN_NUM_THREADS = Threads)
Nchains <- 4
Ncores <- 4
...
```{r GrainSize, cache = TRUE, warning = FALSE, eval = FALSE}
...
```{r DataMpdRegr, cache = TRUE, warning = FALSE, eval = FALSE}
...
Calculate partial $R^{2}$ of the effects of shifts, time and environment by subtracting from the $R^{2}$ of the full model including all predictors (i.e. `r FitMpd`) the $R^{2}$ of a model without shifts (i.e. `r MpdNoShiftRegr`), without the linear relationship with time (i.e. `r MpdNoTimeRegr`), and without environmental predictors (i.e. `r MpdNoEnvRegr`).
