...
## 2.2 MPD determination for assembly processes

### 2.2.2 Assessing trait evolution and deterministic assembly processes
...
## 2.3 Assessing morphospace occupation patterns
...
## 2.4 Local paleoenvironmental, climatic, and intrinsic biological effects on phylogenetic community structure 

### 2.4.1 Local paleoenvironmental predictors

We derived the following local paleoenvironmental predictors from the sediment record: total inorganic carbon (TIC), total organic carbon (TOC), potassium (K) counts from XRF scanning, relative sedimentary quartz content, percentages of arboreal pollen excluding Pinus pollen, percentage of pollen from deciduous oaks (Wagner et al., 2019), oxygen isotope composition of the lake water ($\delta^{18}$O<sub>lakewater</sub>), and grain size (Wilke et al., 2020). In order to reduce the noise of these predictors in the regression analysis, we used Gaussian process smoothing, suggested by Simpson (2018) for sediment records. We used the implementation in the R package ‘brms’ 2.15.0 (Bürkner, 2018), which itself is an interface to the Bayesian inference machinery of stan 2.21 (Stan Development Team, 2021). 


#### Aboreal pollen and oaks

For the Gaussian process smoothing of the relative abundance of aboreal pollen and oaks, we used a binomial distribution for the percentage of specific taxa from the total pollen count. To account for variations in sedimentation rate, we assigned weights to each sample of the paleoenviromental predictor representing time captured by the sample (Simpson, 2018). All smoothing models were inferred through Hamiltonian Monte Carlo sampling with four parallel chains (details in Table S6). Through this process we achieved the same temporal resolution for fossil and local paleoenvironmental data. We then matched paleoenvironmental predictors with MPD via their age.
...
Residuals were tested for autocorrelation using the Moran’s I index for irregularly-spaced time series (see Legendre & Gauthier, 2014).
...
For the Gaussian process smoothing of quartz, TIC, and TOC, we used a beta distribution for quartz, TIC, and TOC because they were quantified as the relative quartz content or the share from a given mass of sediment sample.
...
For the Gaussian process smoothing of the oxygen isotopes, we used a Gaussian distribution for the unbounded isotope values.
...
For the Gaussian process smoothing of the Potassium counts, we used a zero truncated Gaussian distribution for the potassium counts. The latter was used instead of a count distribution (e.g. Poisson or negative binomial) because we calculated running means for a window size of 21 samples and therefore used real numbers greater than zero in the Gaussian process smoothening instead of count (i.e. natural numbers). This procedure reduces the temporal resolution of potassium by factor 20 (down from 170,000 values sampled every 7 years). However, we informed the Gaussian process smoothing on the standard error of the running means through the se argument of ‘brms’ as the averaging reduces the temporal variance in potassium counts.
...
For the Gaussian process smoothing of the grain size distribution, we used a Dirichlet distribution for the multivariate grain size composition because they are initial quantified as relative share and sum to 1. Grain size composition was the only multivariate local paleoenvironmental predictor and using all seven grain size categories to test for an influence on phylogenetic community structure increases the chances that one of them is identified to predict community structure. We therefore used the R package ‘robCompositions’ 2.3.1 (Filzmoser et al., 2018) to summarize grain size composition as the first principal component after an isometric logarithmic ratio transformation of the smoothened grain sizes.
...
### 2.4.2 Climatic predictors

In addition to the predictors derived from the sediment record, we included the following climatic predictors in the regression analysis: the global benthic $\delta^{18}$O isotope stack LR04 (Lisiecki & Raymo, 2005), $\delta^{18}$O planktonic isotope ratios – Medstack (Wang et al., 2010), and northern hemisphere summer insolation at the latitude of Lake Ohrid (i.e. 41° N). 

#### Oxygen isotope stacks LR04 and Medstack

To match the MPD values with the age of climatic predictors, LR04 and Medstack were linearly interpolated.
...
Summer insolation was calculated with the R package ‘palinsol’ 0.93 (Crucifix, 2016) according to equations provided by Laskar et al. (2004). To match the ages of the MPD values with summer insolation, summer insolation was calculated with the ages of the assemblages.
...
### 2.4.3 Intrinsic biological predictors

To assess the impact of diatom intrinsic changes (e.g. speciation, extinction, and population dynamics) on assembly processes and to avoid spurious effects of local paleoenvironmental and climatic predictors, we included time as a predictor and allowed for discrete shifts in MPD at a given moment in time. The number of shifts were estimated with a change point analysis in the R package “ecp” 3.1.0 (James & Matteson, 2015). The same number of shifts or less (depending on whether time shifts could be explained by paleoenvironmental and/or climate predictors) were allowed in the regression analysis. Ages of the shifts were not fixed as these might change when inferred together with paleoenvironmental and/or climate influence on MPD.
...
### 2.4.4 Response of deterministic assembly processes to local paleoenvironmental, climatic, and intrinsic biological predictors 

For our regression model, we excluded colinear local paleoenvironmental and climatic predictors from the analysis (pearson correlation > |0.7|) and retained the following predictors: grain size, isotope composition, potassium, TOC, deciduous oaks, Medstack, insolation and time and time shifts as biological predictors.

```{r DataMpdRegr, cache = TRUE, warning = FALSE, fig.width = 10, fig.height = 5, eval = TRUE}
...
Local paleoenvironmental, climatic, and intrinsic predictors were centered to 0 and scaled to a variance of 1 to compare the strength of their influences on MPD (i.e. $\beta$). This will help to compare their influence on the mean phylogenetic distance of the diatom assemblages (MPD).
...
The structure of the regression model is as follows:


where $MPD_{\tau}$ denotes the SES of the mean phylogenetic community distance at time $\tau$, $\beta_{p} X_{p, \tau}$ the linear effect of the pth local paleoenvironmental, climatic, or intrinsic predictor on MPD, $\beta_{S_{k}}$ the parameters for the effects of the $k^{th}$ time shift S (with the maximum of K = 6), $\beta_{S_{7}}$ the intercept, and εt indicates the model error. The notation $\beta_{S_{7}}$ was used for the intercept because the time most distant in the past was defined as the reference level (i.e. from 1.36 Ma until the earliest shift) and all effects $\beta$ of the other shifts are relative to this level.

```{r MpdRegrEquation, cache = TRUE, warning = FALSE, eval = TRUE}
# Response:
#   Mpd: Standardized effect size of the mean phylogenetic distance between diatoms of an assemblage
#        of an assemblage; at 380 moments in time
#   + weights(Weight): Observational weights per diatom sample (time captured by each sample); 
#                      accounts for heteroskedasticity (Simpson, 2018)
# Predictors:
#  Local paleoenvironment:
#  ~ Grainsize: Axis 1 of principle component analyses of smoothened grainsize distribution
#    Iso: Smoothened oxygen isotopes
#    K: Smoothened Potassium counts
#    Oaks: Smoothened relativ abundance of oak pollen
#    Toc: Smoothened TOC concentration
#  Climate:
#    Ins: Insolation at 41°
#    Medstack: planktonic isotope ratios
#  Intrinsic biological predictors:
#    AgeScaled: Age of the diatom sample
#    bS1-bS6: Change in MPD relative to the intercept bS7
MpdRegr <- brms::bf(Mpd | weights(Weight) ~
                      ...
                      bS6 * inv_logit((Age - o5) * 300) * inv_logit((o6 - Age) * 300) + # MPD between Shift 5 & 6
                      bS5 * inv_logit((Age - o4) * 300) * inv_logit((o5 - Age) * 300) + # MPD between Shift 4 & 5
                      bS4 * inv_logit((Age - o3) * 300) * inv_logit((o4 - Age) * 300) + # MPD between Shift 3 & 4
                      bS3 * inv_logit((Age - o2) * 300) * inv_logit((o3 - Age) * 300) + # MPD between Shift 2 & 3
                      bS2 * inv_logit((Age - o1) * 300) * inv_logit((o2 - Age) * 300) + # MPD between Shift 1 & 2
                    ...
                    bGrain + bIso + bK + bOaks + bToc + bIns + bMed + bAge ~ 1,
                    nl = TRUE)
```



```{r MpdRegrPrior, cache = TRUE, warning = FALSE, eval = TRUE}
MpdPrior <- c(# Prior for paleoenvironmental and climatic predictors
              # Centered in 0 (i.e. no effect) and a large variance of 1
              ...
              # Prior for intrinsic biological predictor 
              # (linear change in MPD with age of the diatom assemblage)
              # Centered in 0 (i.e. no effect) and a large variance of 1
              prior(normal(0, 1), nlpar = "bAge"),
              # Prior for the effect of the shifts
              # Centered in 0 (i.e. no effect) and a large variance of 1
              ...
              # Prior for the ages of the shifts
...
We use Bayesian inference to obtain all parameters of the regression model through similar Hamiltonian Monte Carlo sampling using the “brms” package (details in Supporting Information Appendix 5, Table S7). 

```{r MpdRegrInit, cache = TRUE, warning = FALSE, eval = TRUE}
Nchains <- 4 # Run four chains
Ncores <- 4 # All chains in parallel on indiviudal CPU core
Threads <- 4 # Within-chain parallelization (i.e. 4 x 4 cores used)
# Get initial values from where we start Hamiltonian Monte Carlo sampling
Inits <- setInitsRegr(Seed = sample(123456, Nchains), Length = Nchains)
```
...
FitMpd <- brm(MpdRegr,
              ...
              prior = MpdPrior,
              cores = Ncores, chains = Nchains,
              iter = 60000, warmup = 50000, refresh = 0, thin = 2, 
              threads = threading(Threads), backend = "cmdstanr",
              # Save prior draws to assess later their influence on posterior samples
              sample_prior = TRUE, 
              control = list(adapt_delta = 0.999, max_treedepth = 20))
summary(FitMpd)
```

The influence of local paleoenvironmental and climatic predictors on MPD was quantified by calculating a partial $R^{2}$, which is defined as the Bayesian $R^{2}$ (Gelman et al., 2019) of the full regression model minus the $R^{2}$ of a model without inferring the effect of these environmental predictors.
...
**Table S8** Drivers of community assembly of *Cyclotella* sensu lato in Lake Ohrid over the past 1.36 million years. Inferred coefficients ($\beta$) of the Bayesian regression analyses. These related the standardized effect size (SES) of the mean phylogenetic distance (MPD) with local paleoenvironmental, climatic, and intrinsic biological predictors, while inferring shifts in MPD at discrete moments in time (S). Uncertainty is provided as 95% credible interval and a significant influence of local paleoenvironmental and climatic predictors or age on MPD is indicated by ranges not overlapping with zero (highlighted in bold). All local and climatic predictors were centered to 0 and scaled to a variance of 1 before the analysis. Parameters with the numerical index $k$ represent the change in MPD at shift $k$ towards the past, while $\beta_{S_{7}}$ is the MPD from the earliest shift until the next younger one. Note that the shift times are the differences of the parameters $S_{1...k}$ shown in Equation 1 in the manuscript.
...
```{r Caterpillar, cache = TRUE, fig.width = 10, fig.height = 5, eval = TRUE}
DrawsShiftEffects <- data.frame(bS7 = FitMpdDraws$nlpars$bS7$fe$b[, 1],
                                ...
                                bS2 = FitMpdDraws$nlpars$bS2$fe$b[, 1],
                                bS1 = FitMpdDraws$nlpars$bS1$fe$b[, 1])
DrawsShiftAges <- data.frame(S6 = S6trans,
                             S5 = S5trans,
                             S4 = S4trans,
                             S3 = S3trans,
                             S2 = S2trans,
                             S1 = S1trans)
DrawsEnvEffects <- data.frame(bMed = FitMpdDraws$nlpars$bMed$fe$b[, 1],
                              ...
                              bAge = FitMpdDraws$nlpars$bAge$fe$b[, 1])
...
caterpillar(Draws = DrawsShiftEffects, Prob = c(0.75, 0.95), LwdProb = LwdProb,
            ...
            AddDensity = TRUE, ColDensity = "grey")
# Add label y-axis
text(x = -3.3, y = 4.5, labels = "Effect on MPD", srt = 90, xpd = TRUE)
# Add panel index
...
caterpillar(Draws = DrawsEnvEffects, Prob = c(0.75, 0.95), LwdProb = LwdProb,
            Xlab = expression(beta), Ylab = "",
            Yticks = c("Medstack", "Insolation", 
                       "TOC", "Deciduous oaks", "Potassium", "Isotope", "Grain size", 
                       "Age"),
            ColLines = "black", 
            AddDensity = TRUE, ColDensity = "grey")
# Add label y-axis
...
layout(matrix(1:2, nrow = 2), heights = c(0.45, 0.55))
...
**FIGURE 4** Phylogenetic community structure and functional richness in Lake Ohrid over time. (a) Phylogenetic community structure among *Cyclotella* sensu lato species; greenish dots showing distinct periods of environmental filtering, reddish dots phases of species interaction, and blue dots phase of random assembly. The shaded polygon indicates the effect of taxon sampling. (b) Green curve shows the accumulation of functional richness, indicating small (< 0) or large (> 0) occupancy of niche space. The shaded polygon displays the 95% range for a rarefied taxon sampling. Vertical grey and white bars indicate glacial and interglacial periods, respectively.
...
CondEffAge <- conditional_effects_forked(FitMpd, effects = "Age", 
                                         same_as_effects = "AgeScaled", resolution = 400)
...
**FIGURE 5** Response plots showing the relationship between phylogenetic community structure and (a) time with a linear increase and discrete shifts (i.e. intrinsic population dynamics of communities); predicted by the regression model, (b) grain size (high SES MPD = environmental filtering; low SES MPD = species competition), and (c) $\delta^{18}$O<sub>lakewater</sub> (high SES MPD = environmental filtering; low SES MPD = species competition). The individual response plots were generated by keeping the other variable at its mean level. The regression line is depicted by a solid line and the 95% credible intervals by shaded polygons. X-axis values were back transformed to the observed scale after using values center in 0 and scaled to a variance of 1 for the analysis.
...
axis(side = 4, at = seq(0, 0.5, length.out = 6), label = seq(0, 0.005, length.out = 6),
     col.ticks = "grey", col.axis = "grey") # Add ticks to the right axis
axis(side = 1, at = seq(-50, 0, length.out = 5),
     labels = c("0.050", "0.0375", "0.0250", "0.0125", "0.00")) # Add tick-labels to the x-axis
text(x = par("usr")[2] + 11, y = 0.25, labels = "Prior density", col = "grey",
     xpd = TRUE, srt = -90) # Add label to the axis on the right
lines(DenPrior$x, DenPrior$y * 100, col = "grey") # Draw line for prior density
rect(xleft = par("usr")[1], ybottom = 0.54, xright = par("usr")[2], ytop = par("usr")[4],
       col = "grey70", border = "black") # Draw grey rectangle as background for the plot title
text(x = -25, y = 0.555, labels = "Shift 1", adj = c(0.5, 0.5)) # Plot title
...
**Figure S3** Comparing prior and posterior distribution of the ages of discrete shifts in mean phylogenetic distance of *Cyclotella* sensu lato assemblages in Lake Ohrid. Black lines show the sampled parameters for the ages of the shifts and grey lines the prior values for the ages, which we logged during the sampling phase of the Bayesian inference. All posterior and prior values were transformed after the analysis to our time frame of 1.36 Ma because for all shifts except the earliest one, the parameters are specified in the analysis to be the time difference between two consecutive shifts. This model specification constrained shift $S_{k-1}$ to be more recent then shift $S_{k}$.

```{r FigureS4, cache = TRUE, warning = FALSE, fig.width = 11, fig.height = 9, eval = TRUE}
...
# Posterior draws for the effect of shift 1 (i.e. MPD from Shift 1 until the present)
bS1 <- FitMpdDraws$nlpars$bS1$fe$b[, 1] 
# Posterior draws for the effect of shift 2 (i.e. MPD from Shift 2 until Shift 1)
bS2 <- FitMpdDraws$nlpars$bS2$fe$b[, 1] 
bS3 <- FitMpdDraws$nlpars$bS3$fe$b[, 1]
bS4 <- FitMpdDraws$nlpars$bS4$fe$b[, 1]
bS5 <- FitMpdDraws$nlpars$bS5$fe$b[, 1]
...
**Figure S4** Comparing prior and posterior distribution of the effects of discrete shifts on mean phylogenetic distance of Lake Ohrid *Cyclotella* sensu lato assemblages. Black lines show the sampled parameters for the offset in MPD caused at the age of a shift in comparison to the value of MPD between 1.36 Ma and the earliest shift (i.e. Shift 6). Grey lines indicate the prior values for these parameters, which we logged during the sampling phase of the Bayesian inference.

```{r FigureS5, cache = TRUE, warning = FALSE, fig.width = 11, fig.height = 9, eval = TRUE}
...
plotPriorVerusPosterior(Prior = bAgePD, Post = bAge,
                        DensFrom = -2, DensTo = 2, DensN = 2024,
...
**Figure S5** Comparing prior and posterior distribution of the effects of discrete shifts on mean phylogenetic distance of Lake Ohrid *Cyclotella* sensu lato assemblages. Black lines show the sampled parameters for the effect of local paleoenvironmental, climatic, and intrinsic biological predictors on MPD. Grey lines indicate the prior values for these parameters, which were logged during the sampling phase of the Bayesian inference.
